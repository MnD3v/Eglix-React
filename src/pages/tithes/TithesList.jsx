...
export default function TithesList() {
    ...
    const [filters, setFilters] = useState({ payment_method: '' });
    ...
    const loadTithes = useCallback(async () => {
        ...
        try {
            setLoading(true);
            const { data } = await titheService.getAll(currentChurch.id);
            const statsData = await titheService.getStats(currentChurch.id);
            setTithes(data);
            setStats(statsData);
        } catch (err) {
            setError('Erreur lors du chargement des dîmes');
            console.error(err);
        } finally {
            setLoading(false);
        }
    ...
    }, [currentChurch, loadTithes]);

    const handleDeleteClick = (e, tithe) => {
        e.stopPropagation();
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            await titheService.delete(titheToDelete.id);
            setTithes(tithes.filter(t => t.id !== titheToDelete.id));
            // Update stats locally
            const newStats = { ...stats };
            newStats.total = Math.max(0, newStats.total - 1);
            newStats.totalAmount = Math.max(0, newStats.totalAmount - parseFloat(titheToDelete.amount || 0));
            setStats(newStats);
        } catch (error) {
    ...
    const filteredTithes = tithes.filter(tithe => {
        ...
        const matchesStartDate = filters.startDate ? titheDate >= new Date(filters.startDate) : true;

        let matchesEndDate = true;
        if (filters.endDate) {
            const end = new Date(filters.endDate);
            end.setHours(23, 59, 59, 999);
            matchesEndDate = titheDate <= end;
        }
    ...
    const chartData = useMemo(() => {
        if (!filteredTithes.length) return [];

        const sorted = [...filteredTithes].sort((a, b) => new Date(a.date) - new Date(b.date));
    ...
    return (
        <div className="space-y-8 font-sans pb-12 max-w-full overflow-hidden">
            ...
            <div className="flex flex-col ">
                <div>
                    <h1 className="text-3xl md:text-4xl font-serif font-bold text-gray-900 tracking-tight">Dîmes</h1>
            ...
            {chartData.length > 0 && (
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 h-80">
                    <h3 className="text-lg font-bold text-gray-900 mb-4">Évolution des dîmes</h3>
            ...
            <div className="md:hidden space-y-3">
                {filteredTithes.map((tithe) => (
                    <div
                        key={tithe.id}
                        onClick={() => navigate(`/tithes/${tithe.id}`)}
                    ...
                    >
                        <div className="flex justify-between items-start mb-3">
                            ...
                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${tithe.payment_method === 'cash' ? 'bg-blue-50 text-blue-700 border border-blue-100' :
                                tithe.payment_method === 'mobile' ? 'bg-purple-50 text-purple-700 border border-purple-100' :
                                    tithe.payment_method === 'bank' ? 'bg-amber-50 text-amber-700 border border-amber-100' :
                        ...
                        <div className="flex justify-between items-center text-xs text-gray-500">
                            <span>{new Date(tithe.date).toLocaleDateString('fr-FR')}</span>
                            {tithe.description && <span className="truncate ml-2">{tithe.description}</span>}
                        </div>
                    </div>
                ))}
            </div>
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {filteredTithes.map((tithe) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="text-sm font-bold text-gray-900">{formatCurrency(tithe.amount)}</div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${tithe.payment_method === 'cash' ? 'bg-blue-50 text-blue-700 border border-blue-100' :
                                            tithe.payment_method === 'mobile' ? 'bg-purple-50 text-purple-700 border border-purple-100' :
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-500 hidden lg:table-cell">
                                        <div className="truncate max-w-xs">{tithe.description || '-'}</div>
                                    </td>
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            ...
                                            <button
                                                onClick={(e) => handleDeleteClick(e, tithe)}
            ...
            {filteredTithes.length === 0 && (
                <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-gray-200">
                    ...
                    <h3 className="text-xl font-bold text-gray-900">Aucune dîme enregistrée</h3>
                    <p className="text-gray-500 mt-2 mb-6">Commencez par ajouter votre première dîme.</p>
                </div>
            )}

            <FixedButton
                to="/tithes/new"
                text="Ajouter une dîme"
                icon={
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <ConfirmModal
                ...
                title="Supprimer la dîme"
                message={`Êtes-vous sûr de vouloir supprimer cette dîme de ${formatCurrency(titheToDelete?.amount || 0)} ? Cette action est irréversible.`}
