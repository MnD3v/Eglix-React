...
export default function OfferingDetails() {
    ...
    const loadOffering = useCallback(async () => {
        ...
        try {
            ...
            if (data) {
                ...
                setError("Offrande introuvable");
            }
        } catch (err) {
            console.error("Error loading offering:", err);
            setError("Impossible de charger les détails de l'offrande.");
        } finally {
            setLoading(false);
        }
    }, [id]);
    ...
    // ... (début du composant OfferingDetails)
    ...
    if (error || !offering) return (
        <div className="text-center py-20">
            ...
            <p className="mt-2 text-gray-500">{error || "Offrande introuvable"}</p>
            <button
        ...
        <div className="max-w-3xl mx-auto pb-20 font-sans">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">Ref: #{offering.id.toString().padStart(6, '0')}</p>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div className="p-8 border-b border-gray-100 bg-gray-50/50 text-center">
                    ...
                    <h2 className="text-4xl font-bold text-gray-900">{formatCurrency(offering.amount)}</h2>
                    <div className="mt-4 inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white border border-gray-200 text-sm font-medium text-gray-700 shadow-sm">
                        <span className={`w-2 h-2 rounded-full ${offering.payment_method === 'cash' ? 'bg-blue-500' :
                            offering.payment_method === 'mobile' ? 'bg-purple-500' :
                                offering.payment_method === 'bank' ? 'bg-amber-500' :
                                    'bg-gray-500'
                            }`}></span>
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-base text-gray-900">
                                        {offering.description || <span className="text-gray-400 italic">Aucune description</span>}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            {offering.members ? (
                                <div className="flex items-start gap-4 p-4 rounded-xl bg-gray-50 border border-gray-100">
                                    <div className="shrink-0 h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-sm">
                                        {offering.members.first_name[0]}{offering.members.last_name[0]}
                                    </div>
                                    <div>
                                        <p className="text-sm font-bold text-gray-900">
                                            {offering.members.first_name} {offering.members.last_name}
                                        </p>
                                        {offering.members.email && (
                                            <p className="text-xs text-gray-500">{offering.members.email}</p>
                                        )}
                                        {offering.members.phone && (
                                            <p className="text-xs text-gray-500">{offering.members.phone}</p>
                                        )}
                                        <Link
                                            to={`/members/${offering.member_id}`}
                ...
                <div className="px-8 py-4 bg-gray-50 border-t border-gray-100 flex justify-between text-xs text-gray-400">
                    <span>Ajouté le {new Date(offering.created_at).toLocaleDateString('fr-FR')}</span>
                    <span>Dernière modification : {new Date(offering.updated_at).toLocaleDateString('fr-FR')}</span>
                </div>
            </div>
            ...
            <ConfirmModal
                ...
                message="Êtes-vous sûr de vouloir supprimer cette offrande ? Cette action est irréversible."
