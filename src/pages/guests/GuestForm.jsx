...
export default function GuestForm() {
    ...
    const loadGuest = useCallback(async () => {
        ...
        try {
            ...
            alert('Erreur lors du chargement de l\'invité');
        } finally {
            setLoading(false);
        }
    }, [id]);
    ...
    const handleFileChange = async (e) => {
        try {
            ...
            const publicUrl = await guestService.uploadPhoto(file);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEditMode) {
                await guestService.update(id, formData);
            } else {
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                <div>
                    ...
                    <h1 className="text-3xl font-bold text-gray-900 tracking-tight">
                        {isEditMode ? 'Modifier l\'invité' : 'Nouvel invité'}
                    </h1>
                    <p className="text-gray-500 mt-1">Remplissez les informations ci-dessous pour {isEditMode ? 'mettre à jour' : 'créer'} un invité.</p>
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                        ...
                        <div className="space-y-4">
                            <SelectGroup
                                ...
                                options={[
                                    { value: 'first', label: 'Première visite' },
                                    { value: 'second', label: 'Seconde visite' },
                                    { value: 'third', label: 'Troisième visite' },
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <InputGroup label="Date de visite" name="visit_date" type="date" value={formData.visit_date} onChange={handleChange} />
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">Notes de suivi</label>
                            <textarea
                                name="notes"
                                rows={4}
                                ...
                                onChange={handleChange}
                                className="block w-full px-4 py-3 rounded-xl border-gray-200 bg-gray-50 text-gray-900 focus:bg-white focus:border-gray-300 focus:ring-2 focus:ring-gray-100 transition-all duration-200 sm:text-sm placeholder-gray-400"
                                placeholder="Ajoutez des notes sur cet invité..."
                            />
                        </div>
                    </div>
                </div>
