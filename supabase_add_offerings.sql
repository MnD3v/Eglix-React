-- ============================================================
-- OFFERINGS — Offrandes globales d'une église
-- ============================================================
...
-- 4. Créer les politiques RLS (avec suppression préalable pour éviter les conflits d'exécution)
DROP POLICY IF EXISTS "Users can view offerings from their churches" ON public.offerings;
...
DROP POLICY IF EXISTS "Users can insert offerings to their churches" ON public.offerings;
...
DROP POLICY IF EXISTS "Users can update offerings from their churches" ON public.offerings;
CREATE POLICY "Users can update offerings from their churches" ON public.offerings
    FOR UPDATE
    USING (church_id IN (SELECT church_id FROM public.church_users WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS "Users can delete offerings from their churches" ON public.offerings;
CREATE POLICY "Users can delete offerings from their churches" ON public.offerings
    FOR DELETE
    USING (church_id IN (SELECT church_id FROM public.church_users WHERE user_id = auth.uid()));

-- 5. Créer le trigger pour updated_at (utilise la fonction existante des dîmes/groupes)
